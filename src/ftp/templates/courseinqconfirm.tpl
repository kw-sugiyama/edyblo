...
<head>
	...
	<meta name="Description" content="<?=htmlspecialchars($_POST['title'])?>へのお問い合せ内容確認画面｜<?=htmlspecialchars($obj_login->clientdat[0]['sc_introduce'])?>" />
	...
	<title><?=htmlspecialchars($_POST['title'])?>へのお問い合せ内容確認画面｜<?=htmlspecialchars($obj_login->clientdat[0]['cl_jname'])?> <?=htmlspecialchars($obj_login->clientdat[0]['cl_kname'])?></title>
<?=$param_meta_robots?>
</head>
			...
			<div id="main"><!--main start-->
				...
				<div id="mainright"><!--mainright start-->
					...
					<div class="box"><!--box start-->
						<h3 class="white">コースお問い合せ内容の確認</h3>
					</div><!--box end-->
