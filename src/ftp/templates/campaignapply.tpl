...
<head>
	...
	<meta name="Description" content="<?=htmlspecialchars($obj_campaign->campaindat[0]['cp_title'])?>への申し込み | <?=htmlspecialchars($obj_login->clientdat[0]['sc_introduce'])?>" />
...
<title><?=htmlspecialchars($obj_campaign->campaindat[0]['cp_title'])?>への申し込み｜<?=htmlspecialchars($obj_login->clientdat[0]['cl_jname'])?> <?=htmlspecialchars($obj_login->clientdat[0]['cl_kname'])?></title>
<?=$param_meta_robots?>
</head>
<body>
			...
			<div id="main"><!--main start-->
				...
				<div id="mainright"><!--mainright start-->
					...
					<div class="box"><!--box start-->
						<h3 class="white">お申込み</h3>
					</div><!--box end-->

					<?=$campaign_list?>

					<form method="post" action="<?=_BLOG_SITE_URL_BASE?>campaign-apply-confirm/" name="client">
					<input type="hidden" name="mailtype" value="APPLICATE" />
