...
<head>
	...
	<meta name="Description" content="<?=htmlspecialchars($obj_course->coursedat[0]['cs_title'])?>の資料請求｜<?=htmlspecialchars($obj_login->clientdat[0]['sc_introduce'])?>" />
	...
	<title><?=htmlspecialchars($obj_course->coursedat[0]['cs_title'])?>の資料請求｜<?=htmlspecialchars($obj_login->clientdat[0]['cl_jname'])?> <?=htmlspecialchars($obj_login->clientdat[0]['cl_kname'])?></title>
<?=$param_meta_robots?>
</head>
			...
			<div id="main"><!--main start-->
				...
				<div id="mainright"><!--mainright start-->
					...
					<div class="box"><!--box start-->
						<h3 class="white">コース資料請求</h3>
					</div><!--box end-->
		
					

					<form method="post" action="<?=_BLOG_SITE_URL_BASE?>course-req-confirm/" name="client">
					
						<input type="hidden" name="mailtype" value="COURSEREQ" />
