...
<head>
	...
	<meta name="Description" content="<?=htmlspecialchars($obj_category->categorydat[0]['cg_stitle'])?> | <?=htmlspecialchars($obj_login->clientdat[0]['sc_introduce'])?>" />
	...
	<title><?=htmlspecialchars($obj_category->categorydat[0]['cg_stitle'])?>｜<?=htmlspecialchars($obj_login->clientdat[0]['cl_jname'])?> <?=htmlspecialchars($obj_login->clientdat[0]['cl_kname'])?></title>
<?=$param_meta_robots?>
</head>
			...
			<?php require_once( SYS_PATH."templates/header.tpl" ); ?>
				...
				<div id="main"><!--main start-->
					...
					<div id="mainright"><!--mainright start-->
						...
						<!--topicpath start-->        
						<p class="topicpath"><a href="<?=_BLOG_SITE_URL_BASE?>">TOP</a><span class="spacefortopicpath">&#8250;</span><?=htmlspecialchars($obj_category->categorydat[0]['cg_stitle'])?></p>
						<!--topicpath end-->
