...
<head>
	...
	<meta name="Description" content="<?=htmlspecialchars($obj_campaign->campaindat[0]['cp_title'])?>へのお問い合わせ確認画面｜<?=htmlspecialchars($obj_login->clientdat[0]['sc_introduce'])?>" />
	...
	<title><?=htmlspecialchars($obj_campaign->campaindat[0]['cp_title'])?>へのお問い合わせ確認画面｜<?=htmlspecialchars($obj_login->clientdat[0]['cl_jname'])?> <?=htmlspecialchars($obj_login->clientdat[0]['cl_kname'])?></title>
<?=$param_meta_robots?>
</head>
