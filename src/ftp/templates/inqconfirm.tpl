...
<head>
	...
	<meta name="Description" content="お問合せ確認画面｜<?=htmlspecialchars($obj_login->clientdat[0]['sc_introduce'])?>" />
	...
	<title>お問合せ確認画面｜<?=htmlspecialchars($obj_login->clientdat[0]['cl_jname'])?> <?=htmlspecialchars($obj_login->clientdat[0]['cl_kname'])?></title>
<?=$param_meta_robots?>
</head>
