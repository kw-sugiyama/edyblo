...
<head>
		 ...
		 <title>お問い合わせ確認画面｜<?=mb_convert_encoding(htmlspecialchars($obj_login->clientdat[0]['cl_jname']),"SJIS","EUC-JP")?> 
...
<?=mb_convert_encoding(htmlspecialchars($obj_login->clientdat[0]['cl_jname']),"SJIS","EUC-JP")?> <?=mb_convert_encoding(htmlspecialchars($obj_login->clientdat[0]['cl_kname']),"SJIS","EUC-JP")?>へのお問い合わせ</td></tr></table>
	 <font size="<?=FONT_SIZE?>">お問い合わせ確認画面
<!--mainright start-->
<font size="<?=FONT_SIZE?>">
<?=mb_convert_encoding($inquiry_checkbox,"SJIS","EUC-JP")?>
<font size="<?=FONT_SIZE?>">
<?=mb_convert_encoding($request_checkbox,"SJIS","EUC-JP")?>
<font size="<?=FONT_SIZE?>">
<?=mb_convert_encoding($inquiry_box,"SJIS","EUC-JP")?>
<font size="<?=FONT_SIZE?>">
