...
<head>
	...
	<meta name="Description" content="<?=htmlspecialchars($_POST['title'])?>の資料請求内容確認画面｜<?=htmlspecialchars($obj_login->clientdat[0]['sc_introduce'])?>" />
	...
	<title><?=htmlspecialchars($_POST['title'])?>の資料請求内容確認画面｜<?=htmlspecialchars($obj_login->clientdat[0]['cl_jname'])?> <?=htmlspecialchars($obj_login->clientdat[0]['cl_kname'])?></title>
<?=$param_meta_robots?>
</head>
