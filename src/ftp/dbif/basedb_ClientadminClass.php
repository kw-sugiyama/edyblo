...
class basedb_ClientClassTblAccess extends dbcom_DBcontroll {
	...
	function basedb_CheckClient () {
		...
		// ログイン情報取得
		$strSQL = "";
		$strSQL .= " SELECT * FROM v_clientadmin ";
	...
	function basedb_GetClient ( $stpos , $getnum ) {
		...
		if( $this->jyoken["cl_pref"] != "" )        $sql_where .= " AND ar_pref = '{$this->jyoken["cl_pref"]}' ";
		if( $this->jyoken["cl_pstat"] != "" )        $sql_where .= " AND ( cl_pstat = 1 OR cl_pstat is null ) ";
		if( $this->jyoken["cl_dokuji_flg"] == 1 ){
			$sql_where .= " AND cl_dokuji_flg = '{$this->jyoken["cl_dokuji_flg"]}' ";
		}else if( $this->jyoken["cl_dokuji_flg"] == 9 ){
			$sql_where .= " AND ( cl_dokuji_flg = 9 OR cl_dokuji_flg is null ) ";
		}
		if( $this->jyoken["cl_advertisement_flg"] == 1 ){
			$sql_where .= " AND cl_advertisement_flg = '{$this->jyoken["cl_advertisement_flg"]}' ";
		}else if( $this->jyoken["cl_advertisement_flg"] == 9 ){
			$sql_where .= " AND ( cl_advertisement_flg = 9 OR cl_advertisement_flg is null ) ";
		}
		if( $this->jyoken["cl_deldate"] != "" ){
	...
	//echo "GetClient_SQL ... [".$strSQL."]<BR>";
		...
		//　全件数取得
		$strSQL = "";
		$strSQL .= " SELECT count(cl_id) FROM v_clientadmin ";
		$strSQL .= $stmt2;
