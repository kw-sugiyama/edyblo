...
$obj_login->clientdat[0]['sc_keywd'] = str_replace( "-" , "," , $obj_login->clientdat[0]['sc_keywd'] );

// サイト説明文
$arrMetaHeader["description"] = str_replace( "\n" , "" , str_replace( "\r" , "\n" , $arrHeaderView["sc_introduce"] ) );

$arrMetaHeader['google_search_console_key'] = '';
$domain = $_SERVER['HTTP_HOST'];
if ($domain == 'nakano.waseiku.com') {
	$arrMetaHeader['google_search_console_key'] = '<meta name="google-site-verification" content="gvTnS6bC5RAhU_0c0kjZUfscBs7l7eGIz2Uuso2cHXk" />';
}
...
if($dispcnt>=0){
	for($cnt=0;$cnt<=3;$cnt++){
		...
		switch($obj_topmenu->menudat[$cnt]['mn_hstat']){
			case 1:
				$topmenu.='<li class="globalmenu'.$urlflag.'"><a href="'.$linkurl[$flag-1].'">'.htmlspecialchars($obj_topmenu->menudat[$cnt]['mn_hname']).'</a></li>
				';
				break;
			case 3:
				$topmenu.='<li class="globalmenu'.$urlflag.'"><a href="'.$obj_topmenu->menudat[$cnt]['mn_hurl'].'">'.htmlspecialchars($obj_topmenu->menudat[$cnt]['mn_hname']).'</a></li>
				';
				break;
			/*default:
...
$sc_logo="";
if($obj_login->clientdat[0]['sc_logo']){
	$sc_logo='<a href="'._BLOG_SITE_URL_BASE.'"><img src="./img_thumbnail.php?w=400&h=90&dir='.$param_cl_logo_path.'&nm='.$arrHeaderView["sc_logo"].'" alt="" /></a>';
}
