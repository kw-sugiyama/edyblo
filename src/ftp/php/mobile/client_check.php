...
if( $obj_login->clientdat[0]["cl_mobile_dokuji_flg"] != 1 && $_GET['dd']==1 ){
	$obj_error->ViewErrMessage( "MENT" , "USER-ALL" , "#" , NULL );
//	echo "ok3";
	exit;
}

// 携帯サイト有効/無効 無効だったらエラー
if( $obj_login->clientdat[0][cl_mobile_flg]==9 or $obj_login->clientdat[0][cl_mobile_flg]=="" ){
	$obj_error->ViewErrMessage( "MENT" , "USER-ALL" , "#" , NULL );
	exit;
}
...
if($obj_login->clientdat[0]["cl_mobile_googlemap_key"]!="" &&
	 $obj_login->clientdat[0]["cl_mobile_dokuji_flg"]==1 &&
	 $obj_login->clientdat[0]["cl_mobile_dokuji_domain"]!="" &&
	 $_GET['dd']==1 ){
	 $param_mobile_api_key = $obj_login->clientdat[0]["cl_mobile_googlemap_key"];
}
...
if( $obj_login->clientdat[0]["cl_dokuji_flg"] == 1 && $obj_login->clientdat[0]["cl_dokuji_domain"] != "" ){
	//PCサイトアドレス
	define( "_PC_BLOG_ADDR_URL" , $obj_login->clientdat[0]["cl_dokuji_domain"] );
}else{
	//PCサイトアドレス
	define( "_PC_BLOG_ADDR_URL" , $param_base_blog_addr_url.'/'.$_GET["cl"]."/" );
}
...
if( $obj_login->clientdat[0]["cl_mobile_dokuji_flg"] == 1 && $obj_login->clientdat[0]["cl_mobile_dokuji_domain"] != "" && $_GET['dd']!=1 ){
	...
	header("Location: {$obj_login->clientdat[0]["cl_mobile_dokuji_domain"]}{$str_url}");
	exit;
}
...
$titletable.='<font color="#663300">';
$jname=htmlspecialchars($obj_login->clientdat[0]['cl_jname']);
$kname=htmlspecialchars($obj_login->clientdat[0]['cl_kname']);
