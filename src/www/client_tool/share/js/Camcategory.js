/*==================================================
    カテゴリー情報登録／修正入力チェック
==================================================*/
function CamcategoryInputCheck( parts , parts2 )
{
	...
	if( parts.elements["cg_stat"][0].checked ){
		if ( parts.cg_stitle.value == "" ) {
			...
			if( ! SpaceCheck( parts.cg_stitle.value ) ) {
				...
				return false;
                        ...
                        if( ! StrCountCheck( parts.cg_stitle.value,20 ) ) {
	...
	if( parts.elements["cg_stat"][0].checked ){
		if ( parts.cg_dispno.value == "" ) {
			alert("表示順を入力して下さい。");
			parts.cg_dispno.focus();
			return false;
		} else {
			if( ! IntValCheck( parts.cg_dispno.value ) ) {
