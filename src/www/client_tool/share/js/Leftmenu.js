/*==================================================
    カテゴリー情報登録／修正入力チェック
==================================================*/
function LeftmenuInputCheck( parts , parts2 )
{
	
	if( ! parts.elements["lm_stat"][0].checked && ! parts.elements["lm_stat"][1].checked ){
		alert("状態を指定して下さい。");
		parts.elements["lm_stat"][0].focus();
		return false;
	}
	
	if( parts.elements["lm_stat"][0].checked ){
		if ( parts.lm_title.value == "" ) {
			alert("タイトルを入力して下さい。");
			parts.lm_title.focus();
			return false;
		} else {
			if( ! SpaceCheck( parts.lm_title.value ) ) {
				alert("タイトルはスペースのみの登録ができません。");
				parts.lm_title.focus();
				return false;
			}
                        if( ! StrCountCheck( parts.lm_title.value,20 ) ) {
	...
	if( parts.elements["lm_stat"][0].checked ){
		if ( parts.lm_dispno.value == "" ) {
			alert("表示順を入力して下さい。");
			parts.lm_dispno.focus();
			return false;
		} else {
			if( ! IntValCheck( parts.lm_dispno.value ) ) {
				alert("表示順は半角数値のみで入力してください。");
				parts.lm_dispno.focus();
				return false;
			}
		}
...
function LeftmenuInputCheck2( parts , parts2 )
{
