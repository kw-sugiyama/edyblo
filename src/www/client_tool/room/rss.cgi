...
//クライアントIDをセット
$_SESSION['_cl_id'] = $argv[1];

//modeをセット
$_POST["mode"] = $argv[2];

define( '_BLOG_SITE_URL_BASE' , $argv[3] );
...
for($rssX=0;$rssX<$rssCnt;$rssX++){
	...
	$rssBuildItemValue .= "      <link>".$param_base_blog_addr_url.$param_base_blog_addr.$obj_rss_buidPURLcode->clientdat[0]["cl_url_code"]."/room-".$obj_rss_buidP->builddat[$rssX]['room_id']."/</link>\n";
	$rssBuildItemValue .= "      <guid>".$param_base_blog_addr_url.$param_base_blog_addr.$obj_rss_buidPURLcode->clientdat[0]["cl_url_code"]."/room-".$obj_rss_buidP->builddat[$rssX]['room_id']."/</guid>\n";
...
$obj_rss_diary_blog->jyoken["diary_del_date"] = 1;
$obj_rss_diary_blog->sort["diary_upd_date"] = 2;				// 並び順 - 部屋情報最終更新日時で降順
list( $rssDiaryBlogCnt , $rssDiaryBlogTotal ) = $obj_rss_diary_blog->basedb_GetDiary ( 1 , 50 );
...
for($rssX=0;$rssX<$rssDiaryBlogCnt;$rssX++){
	...
	$obj_rss_diary_blogURLcode->jyoken["cl_id"] = $obj_rss_buidP->diarydat[$rssX]["diary_cl_id"];
	...
	$rssDiaryItemValue .= "    <item>\n";
	$rssDiaryItemValue .= "      <title>".$obj_rss_diary_blog->diarydat[$rssX]['diary_title']."</title>\n";
	$rssDiaryItemValue .= "      <link>".$param_base_blog_addr_url.$param_base_blog_addr.$obj_rss_diary_blogURLcode->clientdat[0]["cl_url_code"]."/diary-detail-".$obj_rss_diary_blog->diarydat[$rssX]['diary_id']."/</link>\n";
	$rssDiaryItemValue .= "      <guid>".$param_base_blog_addr_url.$param_base_blog_addr.$obj_rss_diary_blogURLcode->clientdat[0]["cl_url_code"]."/diary-detail-".$obj_rss_diary_blog->diarydat[$rssX]['diary_id']."/</guid>\n";
