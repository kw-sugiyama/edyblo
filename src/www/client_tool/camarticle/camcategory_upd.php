...
/*
		...
		$obj_no->jyoken["cg_clid"] = $_POST["cp_id"];
		...
		list($noCnt,$noTotal) = $obj_no->basedb_GetCategory(1,-1);
			...
			$arrOther["ath_comment"] .= $athComment;
                        $obj_error->ViewErrMessage( "INS_ERROR" , "ALL" , "camarticle_main.php" , $arrOther );
                        exit;
                }
...
*/
		...
		$obj_new->categorydat[0]["cg_clid"] = $_POST["cp_id"];
		...
		$suc = $obj_new->basedb_InsCategory();
			...
			$arrOther["ath_comment"] .= "<input type=\"hidden\" name=\"cg_type\" value=\"{$_POST['cg_type']}\" />";
			$arrOther["ath_comment"] .= $athComment;
                        $obj_error->ViewErrMessage( "DISP_NO" , "ALL" , "camarticle_main.php" , $arrOther );
                        exit;
                }
                if( $suc == 10 ){
			$arrOther["ath_comment"] = "";
			...
			$arrOther["ath_comment"] .= "<input type=\"hidden\" name=\"cg_type\" value=\"{$_POST['cg_type']}\" />";
			$arrOther["ath_comment"] .= $athComment;
                        $obj_error->ViewErrMessage( "CATE_TOP_FLG" , "ALL" , "camarticle_main.php" , $arrOther );
                        exit;
                }
		$message = "小見出し情報を登録しました。";
		break;
	...
	case 'EDIT':
		...
		$obj_rev->categorydat[0]["cg_clid"] = $_POST["cp_id"];
		...
		$suc = $obj_rev->basedb_UpdCategory();
			...
			$arrOther["ath_comment"] .= "<input type=\"hidden\" name=\"cg_type\" value=\"{$_POST['cg_type']}\" />";
			$arrOther["ath_comment"] .= $athComment;
                        $obj_error->ViewErrMessage( "UPD_ERROR" , "ALL" , "camarticle_main.php" , $arrOther );
                        exit;
                }
                if( $suc == 1 ){
			$arrOther["ath_comment"] = "";
                        $arrOther["ath_comment"] = "<INPUT TYPE=\"hidden\" NAME=\"stpos\" VALUE=\"{$_POST['stpos']}\">";
			$arrOther["ath_comment"] .= "<input type=\"hidden\" name=\"cg_type\" value=\"{$_POST['cg_type']}\" />";
                        $obj_error->ViewErrMessage( "DOUBLE_UPD" , "ALL" , "camarticle_main.php" , $arrOther );
                        exit;
                }
                if( $suc == 2 ){
			$arrOther["ath_comment"] = "";
	...
	case 'DEL':
		...
		$suc = $obj_del->basedb_DelCategory(0);
			...
			$arrOther["ath_comment"] = "";
                        $arrOther["ath_comment"] = "<INPUT TYPE=\"hidden\" NAME=\"stpos\" VALUE=\"{$_POST['stpos']}\">";
			$arrOther["ath_comment"] .= "<input type=\"hidden\" name=\"cg_type\" value=\"{$_POST['cg_type']}\" />";
                        $obj_error->ViewErrMessage( "DEL_ERROR" , "ALL" , "camarticle_main.php" , $arrOther );
                        exit;
                }
		$message = "指定された小見出し情報を削除しました。";
		break;
...
<HTML>
  ...
  <BODY>
    <div align="center">
      ...
      <form name="form1" action="camarticle_main.php" method="POST"> 
