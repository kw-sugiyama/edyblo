...
<? 
    ...
    if ( $msg_num <= 0 ) { 
        echo "<BR>メッセージは0通です。"; 
	echo '<BR><A HREF="top.pl';echo "?$pwstr"; echo '">TOPメニューへ</A>'; 
        echo "</BODY></HTML>"; 
        exit; 
    } 
    ...
    if ( $top - $maillist_num >= 1 ) { 
       ...
       $nav = $nav . "<A HREF=maillist.pl?$pwstr&mbox=$mbox2&top=$next_top>[次へ]</A>\n"; 
    } 
    if ( $top < $msg_num - 1 ) { 
       $next_top = $top + $maillist_num; 
       $nav = $nav . "<A HREF=maillist.pl?$pwstr&mbox=$mbox2&top=$next_top>[前へ]</A>\n"; 
    } 
?> 
<?=$page?><BR> 
<?=$nav?><HR> 
...
?> 
<A ACCESSKEY="0" HREF=top.pl?<?=$pwstr?>><?=$f_msg?></A><BR> 
<?=$nav?>
</BODY> 
</HTML> 
