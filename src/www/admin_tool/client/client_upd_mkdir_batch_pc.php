...
/*----------------------------------------------------------*/ 
//独自ドメイン　準備用htaceess作成
//独自ドメインが不正に利用されないようにサイトにアクセスする際
//アクセスチェックファイル（ＰＨＰ）client_check.php を利用する
/*----------------------------------------------------------*/ 
...
FOREACH( $obj2->clientdat as $key => $val ) {
    ...
    if($pc_htaccess_fp===flase)exit("ファイルオープン失敗");
    ...
    $htaccess_str .= "\n";
    $htaccess_str .= "# キャンペーンお問い合わせ確認ページ\n";
