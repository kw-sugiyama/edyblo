...
IF( $_GET["fg"] == 4 ){
	$obj5 = new mstdb_LineClassTblAccess;
	$obj5->conn = $obj_conn_mst->conn;
	$obj5->jyoken["line_sta_cd"] = $_GET["lsc"];
	$obj5->sort["line_sta_name"] = 1;
	list( $ret5 , $intCnt5 ) = $obj5->mstdb_GetLine( 1 );
	IF( $ret5 == -1 ){
		$obj_error->ViewErrMessage( "ACCESS" , "ALL" , "window.close();" , NULL );
		exit;
	}
...
FOR( $iX=0; $iX<$intCnt2; $iX++ ){
	$strSel = "";
	IF( $_GET["lp"] == $obj2->linedat[$iX]["line_pref_cd"] ) $strSel = " selected";
...
<HTML>
  <HEAD>
    ...
    <SCRIPT type="text/javascript" src="./share/js/station.js"></SCRIPT>
    <SCRIPT language="javascript">
    	<!--
    	function SetSelectData( parts )
    	{
		if(parts.ls_name.value == "" ){
			alert("駅を設定してください。");
			return false;
		}
	
    		window.opener.document.<?=$fname?>.<?=$station?>.value += '・' + parts.ls_name.value + '駅\n';
		if(window.opener.document.<?=$fname?>.<?=$ln_cd?>.value != "" && window.opener.document.<?=$fname?>.<?=$ln_cd?>.value != null ){
    		...
    		window.opener.document.<?=$fname?>.<?=$ln_cd?>.value += parts.ls_cd.value + '/' + parts.ls_name.value;
	...
	function DelSelectData( parts )
	{
    		...
    		window.opener.document.<?=$fname?>.<?=$station?>.value = "";
    		window.opener.document.<?=$fname?>.<?=$ln_cd?>.value = "";
...
<?=$viewLine?>
      ...
      <TR>
        <TH align="left">　4. 駅を選択</TH>
      </TR>
      <TR>
        <TD>
          <SELECT name="station_name" onChange="PageReload( this.form , 4 );">
            <OPTION value="">----</OPTION>
<?=$viewStation?>
          </SELECT>
        </TD>
...
<?=$viewStaLine?>
      ...
      <TR>
        <TD>
          ...
          <br />
          <INPUT type="hidden" name="l_cd" value="<?=$strHiddenLine?>" />
