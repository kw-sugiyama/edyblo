...
IF( $_GET["fg"] >= 2 ){
	// 指定都道府県の沿線を取得
	$obj3 = new mstdb_ZipcodeClassTblAccess;
	$obj3->conn = $obj_conn_mst->conn;
	$obj3->jyoken["pref_cd"] = $_GET["lp"];
	$obj3->jyoken["distinct"] = 1;
	list( $ret3 , $intCnt3 ) = $obj3->mstdb_GetZipcode( 4 );
	IF( $ret3 == -1 ){
		$obj_error->ViewErrMessage( "ACCESS" , "ALL" , "window.close();" , NULL );
		exit;
	}
...
FOR( $iX=0; $iX<$intCnt3; $iX++ ){
	$strSel = "";
	IF( $_GET["lc"] == $obj3->zipdat[$iX]["addr_cd"] ){
		$strSel = " selected";
		$strHiddenLineName = $obj3->zipdat[$iX]["address2"];
		$strHiddenLine = $obj3->zipdat[$iX]["addr_cd"];
	}
	$viewLine .= "            <OPTION value=\"{$obj3->zipdat[$iX]["addr_cd"]}\"{$strSel}>{$obj3->zipdat[$iX]["address2"]}</OPTION>\n";
}
...
<HTML>
  <HEAD>
    ...
    <SCRIPT type="text/javascript" src="./share/js/area.js"></SCRIPT>
    <SCRIPT language="javascript">
		...
		window.opener.document.<?=$fname?>.<?=$ln_cd?>.value += parts.l_cd.value + '/' + parts.l_name.value;
	...
	function DelSelectData( parts )
	{
    		...
    		window.opener.document.<?=$fname?>.<?=$ln_cd?>.value = "";
    		window.opener.document.<?=$fname?>.<?=$st_cd?>.value = "";
