...
if( $login_val["ad_auth"] != "0" ){
	$obj_error->ViewErrMessage( "ACCESS" , "ALL" , SITE_APTH."blank.php" , NULL );
	basecom_ErrmsgJmp ( "","マスタ情報を更新するには管理権限のあるユーザーでログインして下さい。",NULL );
	exit;
}
...
/*--------------------------------------------------------
  マスタ削除処理部分
--------------------------------------------------------*/
$obj_zip = new mstdb_LineClassTblAccess;
$obj_zip->conn = $obj_conn_mst->conn;
list( $ret , $intDelCnt ) = $obj_zip->mstdb_DelLineAll();
IF( $ret == "-1" ){
	$viewComment = $syserr_msg;
}ELSE{
	$viewComment = $intDelCnt."件の沿線駅情報が削除されました。";
}
