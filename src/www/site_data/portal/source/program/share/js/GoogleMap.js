...
function loadMap(mode,mapX2,mapY2,zoomO,dfX,dfY,dfZ,mkrX,mkrY,mkrName,marker_img,marker_shadow_img) {
	...
	if( (modeData[5] == 1 && mapX2 != '' && mkrX != '' ) || modeData[7] == 1 || modeData[7] == 2 ){
	...
	map = new google.maps.Map(
		...
		{
		     center: new google.maps.LatLng(keido, ido),
		     zoom: Number(18),
	...
	var marker = new google.maps.Marker(
	 {
	     position: new google.maps.LatLng(keido, ido),
	     map: map
	 }
	);
...
/*
  ...
  if( (modeData[5] == 1 && mapX2 != '' && mkrX != '' ) || modeData[7] == 1 || modeData[7] == 2 ){
    ...
    GEvent.addListener(map, 'zoomend', function( oldZoomLevel , newZoomLevel ) {
      ...
      document.getElementById("replaceOpZ").value = newZoomLevel;
      document.getElementById("replaceOpZ2").value = newZoomLevel;
    });
    ...
    if( modeData[7] != 2 ){
      ...
      if(mkr_flg != 9){
        var mp = new GLatLng(keido, ido);
        var mk = new GMarker(mp,iconD);
        map.addOverlay(mk);
...
///////////////////////////////////////////////////////////////
  ...
  if(modeData[1] == 1)map.addControl(new GMapTypeControl());		//航空、地図+写真パネル
  if(modeData[2] == 1)map.addControl(new GOverviewMapControl());	//右下の小さい地図
  ctrlObj = new GScaleControl();					//スケール表示
  map.addControl(ctrlObj);						//スケール表示
  if(modeData[3] == 1){
...
/*------------------------------------------
	住所検索･移動関数
------------------------------------------*/
function showAddress_operation() {
	var point;
	if (geocoder) {
		geocoder.getLatLng( document.getElementById("zip").value, function(point) {
			if (!point) {
				...
				map.setCenter(point);
				var y = (map.getCenter(point)).lng();
				var x = (map.getCenter(point)).lat();
				document.getElementById("replaceOpX").value = x;
				document.getElementById("replaceOpY").value = y;
				document.getElementById("replaceOpX2").value = x;
				document.getElementById("replaceOpY2").value = y;
			}
		});
	}
	var ObjPoint = new GPoint(point);
}
...
/*------------------------------------------
	中心位置移動関数（中心値可変用）
------------------------------------------*/
function replace_operation(mapX2,mapY2,zoomO) {

  if(zoomO != ""){
	var zoom = parseInt(document.getElementById("replaceOpZ").value);
  }else{
	var zoom = 12;
  }
  if(mapY2 == ""){
	...
	var ido = parseFloat(document.getElementById("replaceOpY").value);
  }else if(mapY2 % 1 == 0){
	var ido = parseInt(document.getElementById("replaceOpY").value);
  }
  if(mapX2 == ""){
	var keido = 138;
  }else if(mapX2 % 1 != 0){
	var keido = parseFloat(document.getElementById("replaceOpX").value);
  }else if(mapX2 % 1 == 0){
	var keido = parseInt(document.getElementById("replaceOpX").value);
  }
