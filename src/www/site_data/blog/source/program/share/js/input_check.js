/*=============================================================================
	...
	6.  TellCheck_all( val )		... 電話番号チェック("-"の有無問わず)
	7.  PhsCheck_1( val )			... 携帯番号チェック("-"の有無問わず)
	8.  PhsCheck_2( val )			... 携帯番号チェック("-"必須)
	9.  EmailCheck( val )			... メールアドレスチェック
	10.  PhsEmailCheck( val )		... 携帯メールアドレスチェック
	11. DateCheck_1( val )			... 日付チェック
	12. DateCheck_2( val1 , val2 )		... 日付比較チェック
	13. URLCheck( val )			... URLアドレスチェック
	14. LoginValCheck( val )		... ログインID・PASS用チェック
	15. IntValCheck( val )			... 半角数値のみチェック
	16. DataHankakuCheck( val )		... 半角小文字英数字のみチェック
	17. StrCountCheck( val , int )		... 文字数チェック
	18. ReturnCntCheck( val , int )		... 改行数チェック
	19. StrCountCheck_Equal( val , int )	... 文字数チェック(指定文字数と同じ)
	20. StrKatakanaCheck( val )		... 全角カタカナのみチェック
	21. FormSelectCheck( val , int )	... 指定個数のチェックがされているかチェック
...
/*---------------------------------------------------------
	...
	返り値：true  ... 電話番号として認識できる
		false ... 電話番号として認識できない
---------------------------------------------------------*/
function TellCheck_all( val )
{
	var tel = val.replace(/[━.*‐.*―.*−.*\-.*ー.*\-]/gi,'');
	ret_val = tel.match(/^(0[5-9]0[0-9]{8}|0[1-9][1-9][0-9]{7})$/);
	if ( !ret_val ) {
		return false;
	} else {
		return true;
