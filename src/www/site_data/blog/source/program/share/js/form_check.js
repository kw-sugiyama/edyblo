...
//	if ( parts.tel.value == "" && parts.ctel.value == "" ) {
		...
		// スペースのみチェック
		if ( parts.tel.value != "" ) {
			if ( !SpaceCheck( parts.tel.value ) ) {
				alert("電話番号-電話番号はスペースのみの入力はできません。");
				parts.tel.focus();
				return false;
			}
			if ( !TellCheck_all(parts.tel.value) ) {
				alert("電話番号-電話番号形式で入力してください。");
				parts.tel.focus();
				return false;
			}
		}
/*
		// スペースのみチェック
		if ( parts.ctel.value != "" ) {
			if ( !SpaceCheck( parts.ctel.value ) ) {
				alert("電話番号-携帯番号はスペースのみの入力はできません。");
				parts.ctel.focus();
				return false;
			}
			if ( !PhsCheck_1(parts.ctel.value) ) {
				alert("電話番号-携帯番号形式で入力してください。");
				parts.ctel.focus();
				return false;
			}
		}
